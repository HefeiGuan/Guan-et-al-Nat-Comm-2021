% For defining cell assemblies from left and right direction separately

%% Principal component ananlysis
% Animal is moving in the left direction
...
for p = 1:nPatterns
    assmblPtrnWgts(:,p) = assmblPtrnWgts(:,p)./norm(assmblPtrnWgts(:,p));% why normalized
    ...
    leftspikeTrains(linearInd) = leftassemblyMembership(linearInd(:)) + 1;%whats this for
    end;
...
%% The same as above, when animal is exploring in the right direction
